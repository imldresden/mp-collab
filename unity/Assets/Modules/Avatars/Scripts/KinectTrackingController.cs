...
public class KinectTrackingController : MonoBehaviour
{
    ...
    public bool transmitSkeletons = true;
    ...
    private Guid _kinectId = Guid.NewGuid();
    private bool _dataAvailabe = false;
    private long _time = 0L;
    private int _count = 0;

    private KinectTrackingProvider _skeletalTrackingProvider;
    public KinectDataFrame _lastFrameData = new KinectDataFrame();
    ...
    {
        if (_skeletalTrackingProvider != null)
        {
            _skeletalTrackingProvider.Dispose();
        }
    }

    private void TransmitKinectData(KinectDataFrame trackerFrameData)
    {
        if (_kinectDataService != null)
        {
            var PointCloud = new PointCloudDataFrame()
            {
                Data = trackerFrameData.PointCloud,
                KinectId = trackerFrameData.KinectId,
                RoomId = trackerFrameData.RoomId,
                TimestampInMs = trackerFrameData.TimestampInMs,
            };

            _kinectDataService.SendMessage(new MessagePointCloud(PointCloud));

            var SkeletonData = new SkeletonDataFrame()
            {
                Bodies = trackerFrameData.Bodies,
                KinectId = trackerFrameData.KinectId,
                NumOfBodies = trackerFrameData.NumOfBodies,
                RoomId = trackerFrameData.RoomId,
                TimestampInMs = trackerFrameData.TimestampInMs
            };

            _kinectDataService.SendMessage(new MessageSkeletonData(SkeletonData));
        }
    }

    //private void TransmitSkeletons(Body body)
    //{
    //    NetworkedSkeleton Skeleton = NetworkedSkeleton.DeepCopy(body);

    //    if (_kinectDataService != null)
    //    {
    //        _kinectDataService.SendMessage(new MessageUpdateSkeletonData(new List<NetworkedSkeleton> { Skeleton }));
    //    }
    //}
    ...
    private void Start()
    {
        ...
        _skeletalTrackingProvider = new KinectTrackingProvider(TRACKER_ID);
        ...
        {
            _kinectDataService = _networkServiceManager.StartServer(NetworkServiceDescription.ServiceType.KINECT_DATA, _kinectId.ToString());
            _kinectDataService.RegisterMessageHandler(MessageContainer.MessageType.POINT_CLOUD_REQUEST, PointCloudRequested);
        }

        //StartCoroutine(TransmitData());
    }

    private Task PointCloudRequested(MessageContainer container)
    {
        _skeletalTrackingProvider?.RequestUnfilteredData();
        StartCoroutine(TransmitUnfilteredData());
        return Task.CompletedTask;
    }

    /// <summary>
    /// Coroutine that sends an unfiltered point cloud.
    /// </summary>
    /// <param name="container"></param>
    /// <returns></returns>
    private IEnumerator TransmitUnfilteredData()
    {
        KinectDataFrame dataBuffer = new KinectDataFrame();
        while(true)
        {
            yield return new WaitForSeconds(0.5f);
            _skeletalTrackingProvider.GetCurrentFrameData(ref dataBuffer, false);

            var PointCloud = new PointCloudDataFrame()
            {
                Data = dataBuffer.PointCloud,
                KinectId = dataBuffer.KinectId,
                RoomId = dataBuffer.RoomId,
                TimestampInMs = dataBuffer.TimestampInMs,
            };

            if (PointCloud != null && PointCloud.Count > 100)
            {
                _kinectDataService.SendMessage(new MessageCalibrationPointCloud(PointCloud));
                Debug.Log("Sending unfiltered Point Cloud data!");
                break;
            }            
        }
    }
    ...
    void Update()
    {
        if (_skeletalTrackingProvider.IsRunning) // check that we are running at all
        {
            if (_skeletalTrackingProvider.GetCurrentFrameData(ref _lastFrameData, true)) // try to get new data
            {
                if (_lastFrameData.NumOfBodies != 0) // check if there are any tracked people
                {
                    // set room id if applicable
                    if (_sessionManager != null)
                    {
                        _lastFrameData.RoomId = _sessionManager.Room.Id;
                    }

                    // set sensor id
                    _lastFrameData.KinectId = _kinectId;

                    // send data if the option is enabled
                    if (transmitSkeletons)
                    {
                        TransmitKinectData(_lastFrameData);
                    }
                }
            }
        }
    ...
    int findIndexFromId(KinectDataFrame frameData, int id)
    {
        int retIndex = -1;
    ...
    private int findClosestTrackedBody(KinectDataFrame trackerFrameData)
    {
        int closestBody = -1;
    ...
    public void renderSkeleton(NetworkedBody skeleton, int skeletonNumber)
    {
        for (int jointNum = 0; jointNum < (int)JointId.Count; jointNum++)
        {
