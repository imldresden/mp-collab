...
StructuredBuffer<int> _XBuffer;
StructuredBuffer<int> _YBuffer;
StructuredBuffer<int> _ZBuffer;
...
Varyings Vertex(Attributes input)
{
    ...
    float pos_x = float(_XBuffer[input.vertexID]) / 1000.0;
    float pos_y = float(_YBuffer[input.vertexID]) / 1000.0;
    float pos_z = float(_ZBuffer[input.vertexID]) / 1000.0;
    float4 pos = mul(_Transform, float4(pos_x, pos_y, pos_z, 1));
    half3 col = DecodeColor(_RGBBuffer[input.vertexID]);
